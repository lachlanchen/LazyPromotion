...
| [`app_preview.py`](app_preview.py) and [`apps/`](apps/README.md) | Local web/PWA history and editable campaign drafts with explicit save/export; run `python3 app_preview.py`. Native Android and SwiftUI previews still use a dated public snapshot. Android compiles; iOS compilation is unverified. No automatic publishing yet. |
...
| [`network.py lookup`](docs/source-screening-lookup.md) | Read prior private screening decisions before revisiting a source; no fetching, synchronization or sending |
| [`owned_monitor.py`](owned_monitor.py), [`social_inbox_monitor.py`](social_inbox_monitor.py), [`threads_inbound_monitor.py`](threads_inbound_monitor.py), [`github_inbound_monitor.py`](github_inbound_monitor.py), and [`lkt_inbox.py`](lkt_inbox.py) | Read-only publication monitoring, privacy-limited Instagram/Reddit inbox badges, Threads reply alerts, owned-repository and explicit external-thread activity alerts, and private fit-check intake |
...
| [`mail_inbound_check.py`](mail_inbound_check.py) and [`desktop_lease.py`](desktop_lease.py) | Finite hourly mail-count checks that skip active reviews and close only their own desktop; no message bodies or replies. [Operation and coverage](docs/application-inbox-monitoring.md) |
| [`gmail_application_monitor.py`](gmail_application_monitor.py) | Optional Gmail checks for configured application subjects and sender domains, with account verification and separate status; never opens messages or sends replies |
