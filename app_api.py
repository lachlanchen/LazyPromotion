...
def application(environ, start_response):
    ...
    if not loopback_authority(environ.get("HTTP_HOST", "")):
        return respond(400, {"error": "invalid_host"})
    if method not in {"GET", "HEAD"}:
        return respond(405, {"error": "read_only"}, (("Allow", "GET, HEAD"),))
    ...
    if origin is not None and origin != "http://" + environ["HTTP_HOST"]:
        return respond(403, {"error": "cross_origin_not_supported"})
    if environ.get("HTTP_SEC_FETCH_SITE") == "cross-site":
        return respond(403, {"error": "cross_origin_not_supported"})
    length = environ.get("CONTENT_LENGTH", "")
    if length not in {"", "0"} or environ.get("HTTP_TRANSFER_ENCODING"):
        return respond(400, {"error": "request_body_not_supported"})
    if environ.get("QUERY_STRING"):
        return respond(400, {"error": "query_not_supported"})
