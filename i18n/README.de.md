...
| [`app_preview.py`](../app_preview.py) und [`apps/`](../apps/README.md) | Lokale, schreibgeschützte Web/PWA-Vorschau ausgewählter öffentlicher Kampagnen mit ausdrücklich gespeicherten Offline-Kopien; Start mit `python3 app_preview.py`. Native Clients und automatisches Veröffentlichen sind in dieser Vorschau nicht implementiert. |
...
| [`mail_inbound_check.py`](../mail_inbound_check.py) und [`desktop_lease.py`](../desktop_lease.py) | Stündliche, zeitlich begrenzte Prüfungen der E-Mail-Zähler; laufende Reviews werden übersprungen und nur selbst gestartete Desktops geschlossen. Keine Nachrichteninhalte oder Antworten. [Betrieb und Abdeckung](../docs/application-inbox-monitoring.md) |
| [`gmail_application_monitor.py`](../gmail_application_monitor.py) | Optionale Gmail-Prüfungen für konfigurierte Bewerbungsbetreffe und Absenderdomains, mit Kontoprüfung und eigenem Status; öffnet keine Nachrichten und sendet keine Antworten |
