...
| [`app_preview.py`](../app_preview.py) y [`apps/`](../apps/README.md) | Historial web/PWA local y borradores de campaña editables, con guardado y exportación explícitos; ejecutar `python3 app_preview.py`. Las vistas de Android y SwiftUI siguen usando una copia pública fechada. Android compila; la compilación de iOS no está verificada. Aún no hay publicación automática. |
...
| [`mail_inbound_check.py`](../mail_inbound_check.py) y [`desktop_lease.py`](../desktop_lease.py) | Comprobaciones horarias de duración limitada de los recuentos de correo; omiten las revisiones activas y cierran solo su propio escritorio, sin leer mensajes ni responder. [Uso y cobertura](../docs/application-inbox-monitoring.md) |
| [`gmail_application_monitor.py`](../gmail_application_monitor.py) | Comprobaciones opcionales de Gmail para asuntos de candidaturas y dominios remitentes configurados, con verificación de cuenta y estado independiente; nunca abre mensajes ni envía respuestas |
