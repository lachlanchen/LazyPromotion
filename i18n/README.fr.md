...
Le dépôt tient aussi l’inventaire public de 109 dépôts sources `lachlanchen` non archivés. Il les combine en occasions vérifiables, formulées du point de vue d’un acheteur, dans les domaines du code, des livres, des graphes de connaissances, de la recherche, des médias, de l’apprentissage des langues et de l’IA locale. La revue MCP principale à USD 500 et neuf prestations adjacentes au périmètre fixe servent l’objectif des premiers USD 1 000 vérifiés. Les clics, étoiles, candidatures, réponses et publications en attente ne sont jamais comptés comme du chiffre d’affaires.
...
| [`app_preview.py`](../app_preview.py) et [`apps/`](../apps/README.md) | Historique web/PWA local et brouillons de campagne modifiables, avec enregistrement et export explicites ; lancer `python3 app_preview.py`. Les aperçus Android et SwiftUI utilisent toujours une copie publique datée. Android compile ; la compilation iOS reste non vérifiée. Pas encore de publication automatique. |
...
| [`mail_inbound_check.py`](../mail_inbound_check.py) et [`desktop_lease.py`](../desktop_lease.py) | Vérifications horaires limitées dans le temps des compteurs de courrier ; elles passent leur tour pendant une révision active et ne ferment que leur propre bureau, sans lire les messages ni répondre. [Utilisation et couverture](../docs/application-inbox-monitoring.md) |
| [`gmail_application_monitor.py`](../gmail_application_monitor.py) | Vérifications Gmail facultatives pour les objets de candidature et domaines expéditeurs configurés, avec contrôle du compte et état distinct ; aucun message ouvert ni aucune réponse envoyée |
