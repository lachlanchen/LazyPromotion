...
- クロスポスト認識：同じ著者による同一の長文コピーは一つの正規候補にまとめ、すでに返信済みのコピーを優先します。
- 新しさを既定に：投稿日時と議論量を記録し、30日を超えた投稿は古いものとして下書きを拒否します。
...
既定では 1920×1080 のディスプレイ（`:116`）、VNC ポート `5936`、単一の noVNC ポート `6136` を使用します。キャンペーンとアフィリエイトの全タブは同じ永続 Chrome プロファイルに保持され、復元された各 Chrome ウィンドウはデスクトップ全体に収まり、干渉する切り抜き領域なしで通常どおり切り替えられます。CDP は `127.0.0.1:9436`、セッションは `lazypromotion-browser` です。全サービスはループバックだけにバインドされ、未知の使用中ポートを拒否し、自身の記録で古いと確認できるディスプレイロックだけを削除します。
