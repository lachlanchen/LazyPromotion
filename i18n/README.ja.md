...
このリポジトリは、アーカイブされていない `lachlanchen` の公開ソースリポジトリ 109 件も一覧化し、コード、書籍、知識グラフ、研究、メディア、語学学習、ローカル AI を、買い手の課題を起点とした根拠付きの機会へ組み合わせます。最初の検証済み売上 USD 1,000 に向けて、USD 500 の MCP レビューを主経路とし、範囲を限定した九つの隣接サービス経路を用意しています。クリック、スター、応募、返信、予約投稿は売上として数えません。
...
| [`app_preview.py`](../app_preview.py) と [`apps/`](../apps/README.md) | ローカル Web/PWA で履歴の閲覧、宣伝用下書きの編集、手動保存・エクスポートが可能です。起動は `python3 app_preview.py`。Android と SwiftUI のプレビューは引き続き取得日時付きの公開データを使用します。Android はビルド済み、iOS のビルドは未検証です。自動投稿は未実装です。 |
...
| [`gmail_application_monitor.py`](../gmail_application_monitor.py) | 設定済みの応募件名と送信元ドメインを対象とする任意の Gmail 確認。アカウントを照合し、状態を個別に記録します。メールを開いたり返信したりしません |
