...
既定では 3840×1080 のディスプレイ（`:116`）上に、同じ永続プロファイルを共有する独立した 1920×1080 の Chrome レーンを 2 つ配置します。noVNC はキャンペーン用を `6138`、アフィリエイト用を `6137`、全体表示を `6136` で公開し、CDP は `127.0.0.1:9436`、セッションは `lazypromotion-browser` です。全サービスはループバックだけにバインドされ、未知の使用中ポートを拒否し、自身の記録で古いと確認できるディスプレイロックだけを削除します。
