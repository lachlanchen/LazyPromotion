...
기본 설정은 3840×1080 디스플레이(`:116`) 하나에 동일한 영구 프로필을 공유하는 독립적인 1920×1080 Chrome 레인 두 개를 배치합니다. noVNC는 캠페인 레인을 `6138`, 제휴 레인을 `6137`, 전체 보기를 `6136`에서 제공하며 CDP는 `127.0.0.1:9436`, 세션은 `lazypromotion-browser`입니다. 모든 서비스는 루프백에만 바인딩되며, 알 수 없는 사용 중 포트를 거부하고 자체 기록으로 오래된 것이 확인된 디스플레이 잠금만 정리합니다.
