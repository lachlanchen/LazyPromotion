...
| [`app_preview.py`](../app_preview.py) и [`apps/`](../apps/README.md) | Локальный веб/PWA-просмотр только для чтения; запуск: `python3 app_preview.py`. Нативные версии Android и SwiftUI используют один датированный снимок публичных данных. Android успешно собран; сборка iOS ещё не проверена. Автоматическая публикация пока не реализована. |
