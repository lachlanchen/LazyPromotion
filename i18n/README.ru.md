...
| [`app_preview.py`](../app_preview.py) и [`apps/`](../apps/README.md) | Локальная история в веб/PWA и редактируемые черновики кампаний с явным сохранением и экспортом; запуск: `python3 app_preview.py`. Версии Android и SwiftUI по-прежнему используют датированный снимок публичных данных. Android успешно собран; сборка iOS ещё не проверена. Автоматическая публикация пока не реализована. |
...
| [`mail_inbound_check.py`](../mail_inbound_check.py) и [`desktop_lease.py`](../desktop_lease.py) | Ограниченные по времени ежечасные проверки счётчиков почты: пропускают активную проверку и закрывают только запущенный ими рабочий стол, не читают содержимое писем и не отвечают. [Работа и охват](../docs/application-inbox-monitoring.md) |
| [`gmail_application_monitor.py`](../gmail_application_monitor.py) | Необязательные проверки Gmail по заданным темам заявок и доменам отправителей, с проверкой аккаунта и отдельным статусом; письма не открываются, ответы не отправляются |
