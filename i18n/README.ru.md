...
- Учёт кросспостов: одинаковые развёрнутые копии одного автора объединяются под одним основным кандидатом; приоритет получает уже отвеченная копия.
- По умолчанию только свежее: сохраняются время источника и объём обсуждения; публикации старше 30 дней помечаются устаревшими и не допускаются к созданию черновика.
...
По умолчанию используются один дисплей 1920×1080 (`:116`), один порт VNC `5936` и один порт noVNC `6136`. Все вкладки кампаний и партнёрских программ остаются в одном постоянном профиле Chrome; каждое восстановленное окно Chrome занимает весь рабочий стол и нормально переключается без мешающих обрезанных областей. CDP остаётся на `127.0.0.1:9436`, сессия — `lazypromotion-browser`. Все службы доступны только через loopback. Запуск отклоняет неизвестные занятые порты и удаляет лишь собственную подтверждённо устаревшую блокировку дисплея.
