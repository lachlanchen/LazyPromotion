...
| [`app_preview.py`](../app_preview.py) và [`apps/`](../apps/README.md) | Bản xem trước web/PWA cục bộ chỉ đọc cho các chiến dịch công khai được chọn, cho phép chủ động lưu bản sao ngoại tuyến; chạy `python3 app_preview.py`. Chưa triển khai ứng dụng gốc và đăng bài tự động trong bản xem trước này. |
...
| [`mail_inbound_check.py`](../mail_inbound_check.py) và [`desktop_lease.py`](../desktop_lease.py) | Kiểm tra số lượng thư mỗi giờ với thời gian giới hạn; bỏ qua khi đang duyệt và chỉ đóng desktop do chính lượt kiểm tra khởi động, không đọc nội dung thư hay trả lời. [Vận hành và phạm vi](../docs/application-inbox-monitoring.md) |
| [`gmail_application_monitor.py`](../gmail_application_monitor.py) | Kiểm tra Gmail tùy chọn theo tiêu đề hồ sơ và tên miền người gửi đã cấu hình, xác minh tài khoản và ghi trạng thái riêng; không mở thư hay gửi trả lời |
