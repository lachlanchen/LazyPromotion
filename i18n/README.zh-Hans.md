...
仓库还维护 `lachlanchen` 名下 109 个未归档公开源码仓库的清单，并把代码、书籍、知识图谱、研究、媒体、语言学习和本地 AI 组合成以买方问题为中心、受证据门槛约束的机会。500 美元 MCP 审查主路线和九条相邻的固定范围服务路线共同支持第一个经核实的 1,000 美元目标；点击、Star、申请和排队中的帖子都不算收入。
...
| [`app_preview.py`](../app_preview.py) 和 [`apps/`](../apps/README.md) | 本地 Web/PWA 可查看历史记录、编辑推广草稿并手动保存或导出；运行 `python3 app_preview.py`。Android 和 SwiftUI 原生预览仍使用带日期的公开数据快照。Android 已编译通过，iOS 编译尚未验证。暂不支持自动发布。 |
