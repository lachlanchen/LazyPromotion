...
預設使用一個 1920×1080 顯示器（`:116`）、一個 VNC 連接埠 `5936` 和一個 noVNC 連接埠 `6136`。所有推廣與聯盟分頁保存在同一個持久化 Chrome 設定檔中；復原的每個 Chrome 視窗都會鋪滿桌面，可正常切換，不再被裁成互相干擾的區域。CDP 維持在 `127.0.0.1:9436`，工作階段為 `lazypromotion-browser`。所有服務只綁定回環位址；啟動器拒絕被未知程序占用的連接埠，只清理由自身記錄證明已失效的顯示鎖。
