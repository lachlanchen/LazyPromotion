...
class ApplicationWatchTests(unittest.TestCase):
    ...
    def test_current_campaigns_have_no_automatic_follow_up(self):
        ...
        self.assertEqual(report["summary"]["awaiting_human_reply"], 40)
        ...
        poetry = next(
            item for item in report["applications"]
            if item["campaign_id"] == "bilingual-poetry-freelancer"
        )
        self.assertEqual(poetry["review_after"], "2026-09-23")
        self.assertFalse(poetry["due_for_human_review"])
        ad3 = next(
            item for item in report["applications"]
            if item["campaign_id"] == "ad3-acquisition-freelancer"
        )
        self.assertEqual(ad3["review_after"], "2026-09-23")
        self.assertFalse(ad3["due_for_human_review"])
        physics = next(
            item for item in report["applications"]
            if item["campaign_id"] == "physics-notes-freelancer"
        )
        self.assertEqual(physics["review_after"], "2026-09-23")
        self.assertFalse(physics["due_for_human_review"])
        smashing = next(
            item for item in report["applications"]
        ...
        self.assertIn("wordpress-teaching-site-updates", current_ids)
        self.assertIn(
            "vetto-code-reviewer",
            [item["campaign_id"] for item in report["applications"]],
        )
        ...
        self.assertNotIn(
            "meridial-infrastructure-specialist",
            [item["campaign_id"] for item in report["applications"]],
        )
        self.assertNotIn(
        ...
        self.assertIn(
            "catalyst-wayfare-agent-builder",
            [item["campaign_id"] for item in report["applications"]],
        )
        self.assertIn(
        ...
        self.assertIn(
            "progress-ai-thought-leadership-contributor",
            [item["campaign_id"] for item in report["applications"]],
        )
        self.assertIn(
