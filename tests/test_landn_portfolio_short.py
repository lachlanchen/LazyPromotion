...
class LandnPortfolioShortTests(unittest.TestCase):
    def test_renderer_is_first_party_traditional_chinese_web_demo(self):
        ...
        self.assertIn("concat=n=5:v=1:a=0", source)
        self.assertNotIn("xfade=", source)
        self.assertIn("SLIDE_DURATIONS = (3.8, 5.8, 3.3, 3.3, 3.15)", source)
        self.assertIn("contain_region(Image, inputs[\"mouth_l\"]", source)
        self.assertIn("One contact point. Two air paths.", source)
