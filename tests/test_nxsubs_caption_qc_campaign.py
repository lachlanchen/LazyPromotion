...
class NxSubsCaptionQcCampaignTests(unittest.TestCase):
    def test_prepared_application_keeps_claims_and_revenue_bounded(self):
        campaign = json.loads(
            (ROOT / "campaigns" / "nxsubs-caption-qc-opportunity.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(
        ...
        self.assertIn("LazyEdit", campaign["fit"]["projects"])
        self.assertIn("paid media-localization", campaign["fit"]["not_proven"][0])
        ...
        self.assertFalse(application["certified_translation_claimed"])
